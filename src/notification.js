...
class Notification extends React.Component{
	...
	render(){
		...
		if( notif_list!=[] ){
			...
			var notif_blocks = notif_list.map( (each_notif)=>(
				<NotifBlock
		...
		return(
			...
			<div>
				...
				<div>
					{/* Render top setting bar of notification */}
					<NotifTopOptions allowNotif={this.state.allowNotif}
					notifSwitchCallback={this.notif_switch} 
					deleteAllCallback={this.deleteAllNotifs}
					/>
				</div>
				
				{/* Render notif blocks if not empty */}
				...
				{/* <ShowLoading/> : Display a circle spinner,
					...
					<ShowNothingToShow /> : display if no notification for user
					when server fetch is succesful
				 */}

				{( this.state.isLoading ? <ShowLoading page_num={1} /> : 
					( this.state.isError ? <ShowError /> :
					( notif_list.length==0 && <ShowNothingToShow page_num={1} /> )) 
				)}
